...
**Don't:**
- Use `--dangerously-skip-permissions` when the session will spawn sub-agents — all sub-agents inherit the bypass, including those inside worktrees (`isolation: "worktree"` provides filesystem isolation but NOT permission restriction)
