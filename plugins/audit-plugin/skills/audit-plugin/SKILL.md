...
- **Sweep temp artifacts:** delete every temp dir/file this audit created — benchmark/e2e fixtures, Stage 1 probe dirs, scratch outputs (track paths at creation time). Exception: benchmark transcripts under `~/.claude/projects/` stay — `cleanupPeriodDays` handles them.
