...
- Label-gated auto-merge (optional, for human PRs): copy `assets/workflows/auto-merge.yml` — adding the `automerge` label to a ready (non-draft) PR enables auto-merge (squash), so GitHub merges it the moment `ci-success` goes green. Same prerequisites as Dependabot auto-merge (branch protection + "Allow auto-merge"); also create the `automerge` label (`gh label create`). It prefers a PAT (`RELEASE_PLEASE_TOKEN`) so the merge still triggers release-please, falling back to `GITHUB_TOKEN`.
...
- `assets/` — file + workflow templates to copy and fill: community-health files, `README-header.md`, config files (`commitlint.config.js`, `labeler.yml`, `release-config.yml`), and `workflows/` (ci, release, release-please, dependency-review, dependabot-auto-merge, auto-merge, commitlint, stale, labeler).
