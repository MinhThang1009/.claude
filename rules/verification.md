...
- Two fork mechanisms:
  - `CLAUDE_CODE_FORK_SUBAGENT=1` (env var) → a fork inherits everything the main session has at spawn: full history + system prompt + tools. Officially documented: forks require **v2.1.117+**; from **v2.1.161** `/fork` is on by default, earlier versions need this env var. *(Confirmed 2026-06 — code.claude.com/docs/en/sub-agents §Fork + /en/env-vars; supersedes the earlier "community-only, unverified" note.)*
...
- **Subagent nesting is allowed, up to 5 levels deep** (v2.1.172; a subagent at depth 5 gets no Agent tool — the limit is fixed, not configurable). Forks and other spawned types count toward the cap; a background subagent's depth is **fixed when spawned and unchanged on resume** (**v2.1.187**, not v2.1.178 — that release was nested-dir duplicate-`name` resolution + agent-teams streamlining). Older versions blocked nesting entirely. Prefer shallow chains from the main conversation. *(Corrected 2026-06 vs code.claude.com/docs/en/sub-agents §Spawn nested subagents.)*
