...
```
~/.claude/
├── CLAUDE.md                       # Hướng dẫn global (load mọi session, ~80 dòng)
...
A: REFERENCE.md = ~1250 dòng, ~19k tokens. Nếu auto-load thì hết ~9.5% context window Sonnet 200k (hoặc ~1.9% Opus 1M) mỗi session. REFERENCE để **NGƯỜI** tra cứu (mở trên màn hình thứ 2 / web), KHÔNG để Claude đọc.
