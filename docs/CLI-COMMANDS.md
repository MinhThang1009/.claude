...
| `/effort` | Chỉnh effort level ([docs](https://code.claude.com/docs/en/model-config#adjust-effort-level)) (`low`/`medium`/`high`/`xhigh` Opus 4.7 only/`max`). `/effort auto` reset về model default (không phải level). |
