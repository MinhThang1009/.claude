#!/usr/bin/env bash
# Wrapper minimal cho bash_guard.py.
...
for PY in python3 python; do
  if command -v "$PY" >/dev/null 2>&1 && "$PY" -c 'import sys; sys.exit(0 if sys.version_info >= (3,6) else 1)' >/dev/null 2>&1; then
    exec "$PY" "$HOME/.claude/hooks/bash_guard.py"
  fi
done
# Không có Python 3.6+ → FAIL-CLOSED (block) thay vì silent allow.
# Defense layer phải hoạt động hoặc user biết để cài.
echo "[bash_guard] BLOCKED: Python 3.6+ không tìm thấy trong PATH. Defense layer không thể hoạt động." >&2
echo "[bash_guard] Cài Python 3 (https://www.python.org/) rồi restart Claude Code, hoặc xóa hook này khỏi settings.json." >&2
exit 2
